...
export default class DragSortableView extends Component{
    ...
    endTouch (nativeEvent) {
        ...
        if (this.touchCurItem) {
            // 提取当前项的所有信息到局部变量
            const itemToProcess = { ...this.touchCurItem };
            const index = itemToProcess.index;
            const moveToIndex = itemToProcess.moveToIndex;
            const ref = itemToProcess.ref;
            
            // 生成唯一ID标记当前操作
            const currentId = Date.now();
            this.currentOperationId = currentId;
            ...
            Animated.timing(
                ...
                // 只执行最新操作的回调
                if (this.currentOperationId === currentId) {
                    ref?.setNativeProps({
                        style: { zIndex: defaultZIndex }
                    });
                    this.changePosition(index, moveToIndex);
                    // 只重置当前操作的ID，不直接操作this.touchCurItem
                    if (this.currentOperationId === currentId) {
    ...
    onPressOut () {
        const isMovePanResponder = this.isMovePanResponder // 在异步回调中保存状态快照
        this.isScaleRecovery = setTimeout(()=> {
            // 在缩放中也执行，还原放大
            if ((this.isScaling || isMovePanResponder) && !this.isHasMove) {
                this.endTouch()
            }
        },220)
    }
